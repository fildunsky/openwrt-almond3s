...
* Яркость подсветки регулируется программным ШИМ прямо в драйвере: у MT7621
  нет аппаратного PWM на этом пине, поэтому таймер ядра крутит GPIO 31 на
  250 Гц. Уровень задаётся на странице «Экран» (25/50/75/100 %) или руками:
  `touch_poll dim 0..255`, текущий - `touch_poll level`. Заводская прошивка
  яркость не умела вовсе: её «BackLight Settings» задаёт лишь часы, когда
  подсветка горит. Ночью (в часы ночного режима) заставка дополнительно
  приглушается до трети яркости - активный экран при этом светит как обычно.
