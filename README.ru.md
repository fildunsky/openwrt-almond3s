...
* **Трафик** — RX/TX живой кривой за последние минуты: приём зелёный,
  отдача голубая
...
* **Игры** — эмулятор NES (QuickNES), рисует прямо в панель: экранные кнопки
  по бокам, USB-клавиатура или джойстик в браузере телефона по Wi-Fi
  (открывается по QR-коду). Файлы `.nes` кладутся в `/etc/almond3s/roms`,
  в пакете их нет
* **Настройки** — экран, заставка, ночное расписание, диод, редактор иконок
  и отладка панели одним местом. Заставка **Виджеты** показывает три
  сменяющиеся страницы карточек — часы с погодой, связь и система, — у каждой
  свои шкалы и графики
