...
Экран **Securifi Almond 3S** и **Almond 3** под OpenWrt: драйвер 2.8" панели
(ILI9341 у 3S, ST7789V у Almond 3) с тачем SX8650 и приложение, которое показывает
на нём сеть, модем, Wi-Fi, трафик и погоду. Плата определяется по DT-compatible,
пакеты одни на обе.
...
Almond 3 отличается: панель ST7789V на той же шине, PIC16F1503 без батареи и
модема, передний диод трёхцветный (в системе `red:status`, `green:status`,
`blue:status`, цвет выбирается на странице «Диод»).
