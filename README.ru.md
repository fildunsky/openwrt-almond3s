...
| Пакет | Что это |
|---|---|
| `kmod-lcd-almond3s` | драйвер ядра: фреймбуфер RGB565 в `/dev/lcd`, тач, диод, зуммер и батарея через PIC (PIC16LF1509 на 3S, PIC16F1503 на Almond 3) |
...
* **Терминал** — настоящий login-шелл на панели (`forkpty` + libvterm) с
  экранной клавиатурой: баннер OpenWrt, `ls`, `cd`, правка строки, `top` и
  `vi`. Вывод цветной (16 цветов ANSI и 256-цветный куб), UTF-8 с кириллицей
  и рамками псевдографики, строгая сетка 52×22 клетки по 6 пикселей. `5gtop`
  из luci-app-5gmodem распознаёт маленький экран и укладывается в 22 строки
...
* Панель ST7789V на той же шине (параметр модуля `panel=1`, ставится сам по
  DT-compatible `securifi,almond-3`)
* PIC16F1503 вместо PIC16LF1509, на том же адресе I²C palmbus 0x2A и с тем же
  протоколом команд. Он управляет зуммером и передним диодом и держит кнопку
  питания, но заряд батареи не отдаёт: у Almond 3 нет ни батареи, ни модема
* Передний диод трёхцветный, а не белый. PIC принимает две команды с байтом
  яркости на канал (`0x30` — зелёный и синий, `0x31` — красный); драйвер
  показывает его как три LED-устройства `red:status`, `green:status`,
  `blue:status` (0..255), LuCI видит обычные диоды с триггерами. Параметр
  `led_rgb=1`, тоже по плате. На странице «Диод» восемь предустановок цвета,
  `almond3s-lcd led on RRGGBB` задаёт любой
...
* **Диод** — белый светодиод над экраном: включить, выключить и мигание, пока
  ...
  управляется командой `almond3s-lcd led on|off|blink`. На Almond 3 та же
  страница управляет RGB-диодом: выбор цвета из восьми предустановок,
  `almond3s-lcd led on RRGGBB`
