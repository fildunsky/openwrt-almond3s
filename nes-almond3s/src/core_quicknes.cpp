 ...
 *
 * Второе ядро рядом с InfoNES: выбирается в «Игры → Настройки». Взято ради
 * MMC3-игр (Super Mario Bros 3 и подобные): InfoNES при каждом переключении
 * банка графики перепаковывает данные в буфер размером ровно в кэш процессора
 * и вымывает его целиком, а QuickNES держит распакованными все тайлы картриджа
 * сразу - переключение банка у него это смена смещения.
 */
