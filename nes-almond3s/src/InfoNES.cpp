...
/*                                                                   */
/*===================================================================*/
/* Almond 3S: платформенный слой шагает по одному кадру. Всё состояние
   эмуляции лежит в глобальных переменных, поэтому цикл развёртки можно
   покинуть на границе кадра, а следующий вызов InfoNES_Cycle() продолжит
   ровно с того же места. */
int InfoNES_StepOut;
int InfoNES_FrameDone;
 ...
 */
  ...
  {    
    ...
    if ( InfoNES_StepOut && InfoNES_FrameDone )
    {
      InfoNES_FrameDone = 0;
      return;
    }
  ...
  switch ( PPU_Scanline )
  {
    ...
    case SCAN_UNKNOWN_START:
      if ( FrameCnt == 0 )
      {
        ...
        /* Almond 3S: кадр готов - отмечаем, цикл развёртки выйдет наружу. */
        InfoNES_FrameDone = 1;
